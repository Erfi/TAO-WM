...
@hydra.main(version_base="1.3", config_path="../../config/datamodule/dataset", config_name="calculate_stats")
def calculate_dataset_statistics(cfg: DictConfig) -> None:
    ...
    counter = 0
    """Parse through the whole CALVIN-style data """
    data_path = Path(input_dir)

    # Load episode start and end ids if needed for processing
    ep_start_end_ids = np.load(data_path / "ep_start_end_ids.npy", allow_pickle=True)
    for ep_start, ep_end in tqdm(ep_start_end_ids, desc="Processing episodes"):
        for ep_id in range(ep_start, ep_end + 1):
            ep_id_str = str(ep_id).zfill(cfg.episode_id_length)
            npz_file = data_path / f"episode_{ep_id_str}.npz"
            ...
            rel_action_world_min_bound = np.minimum(rel_action_world_min_bound, data["rel_actions_world"].min(axis=0))
            rel_action_world_max_bound = np.maximum(rel_action_world_max_bound, data["rel_actions_world"].max(axis=0))
    ...
    with open(stats_file, "w") as f:
        for key, value in stats_dict.items():
            if key in ["robot_obs", "scene_obs"]:
                f.write(f"{key}:\n")
                f.write("  - _target_: taowm.utils.transforms.NormalizeVector\n")
