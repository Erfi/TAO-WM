...
class TransformManager(object):
    ...
    def __call__(self, input: dict, transf_type="train", device="cpu"):
        ...
        for modality, value in input.items():
            ...
            if modality in self.transforms[transf_type].keys():
                transf_input[modality] = self.transforms[transf_type][modality](value_tensor)
            else:
    ...
    def image_to_tensor(self, img: np.ndarray, modality: str = "rgb", device: str = "cuda"):
        if "rgb" in modality:
            if len(img.shape) == 3:
        ...
        for transf_type, value in transforms.items():
            ...
            for modality, modality_transf in value.items():
                ...
                for transf in modality_transf:
                    transforms_list.append(hydra.utils.instantiate(transf, _convert_="partial"))
                self.transforms[transf_type][modality] = self.get_transforms(transforms_list)
...
class LinearizeDepth(object):
    ...
    def __call__(self, depth: torch.Tensor) -> torch.Tensor:
        ...
        lin_depth = 2.0 * self.near / (self.far + self.near - z_buffer * (self.far - self.near))
        return lin_depth

    def __repr__(self):
        return self.__class__.__name__ + +"(near={0}, far={1})".format(self.near, self.far)
...
class ColorizeDepth(object):
    ...
    def __call__(self, depth: torch.Tensor) -> torch.Tensor:
        ...
        assert np.issubdtype(depth.dtype, np.floating), "Depth image is assumed to be float"
        assert len(depth.shape) == 3, "Depth image doesn't have the appropiate shape"
        depth = depth.clip(0, 1)
        colorized_depth = self.colormap(depth)[:, :, :, :3]
        colorized_depth = torch.from_numpy(colorized_depth).to(device).permute(0, 3, 1, 2)
...
class ArrayToTensor(object):
    ...
    def __call__(self, array: np.ndarray, device: torch.device = "cuda") -> torch.Tensor:
...
class ColorTransform(object):
    def __init__(self, contrast=0.3, brightness=0.3, hue=0.3, prob=1.0):
        super().__init__()
        self.prob = prob
        self.jitter = transforms.ColorJitter(contrast=contrast, brightness=brightness, hue=hue)
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        assert isinstance(tensor, torch.Tensor), "Input of color transform must be a tensor"
...
def transform_observation(
    ...
    if isinstance(obs, dict) and "goal" in obs:
        transf_obs = transform_manager(obs["observation"], transf_type=transf_type, device=device)
        transf_goal = transform_manager(obs["goal"], transf_type=transf_type, device=device)
