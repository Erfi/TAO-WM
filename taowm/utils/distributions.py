...
from taowm.utils.misc import atanh
...
class GumbelSoftmax(RelaxedOneHotCategorical):
    ...
    def log_prob(self, value):
        ...
        if value.ndim == 0 or value.shape[-1] != self.logits.shape[-1]:
            ...
            assert value.shape[-1] == self.logits.shape[-1], "Last dimension should match after one hot encoding"
...
class TanhNormal(Distribution):
    ...
    def _log_prob_from_pre_tanh(self, pre_tanh_value):
        ...
        correction = -2 * (math.log(2) - pre_tanh_value - F.softplus(-2 * pre_tanh_value)).sum(dim=-1)
