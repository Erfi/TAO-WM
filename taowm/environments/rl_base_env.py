...
from taowm.utils.egl import set_current_egl_device
from taowm.utils.matrix_transforms import to_world_frame
...
class RLBaseEnv(PlayTableSimEnv):
    def __init__(
        ...
        **kwargs,
        ...
        assert len(modalities) > 0, "There must be at least one modality in the observation"
    ...
    def construct_observation_space_from_modalities(self, modalities: List[str] = []):
        ...
        for view in modalities:
            ...
            elif view.startswith("rgb_"):
                observation_space[view] = spaces.Box(
                    low=0,
                    high=255,
                    shape=(self.views_size[view][0], self.views_size[view][1], 3),
                    dtype=np.uint8,
                )
            elif view.startswith("depth_"):
                observation_space[view] = spaces.Box(
                    low=0,
                    high=1,
                    shape=(self.views_size[view][0], self.views_size[view][1], 1),
                )
            else:
                observation_space[view] = spaces.Box(
                    low=-1,
