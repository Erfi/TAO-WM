...
class GoalConditionedEnv(RLBaseEnv):
    ...
    def set_tasks_from_complete_info(
        ...
        super().reset(options=goal_info)
        self.goal = super().get_obs(modalities=self.goal_modalities)
        end_info = self.get_info()
        obs, info = super().reset(options=start_info)
        self.start_info = self.get_info()
        if tasks is None or len(tasks) == 0:
            self.selected_tasks = list(self.tasks.get_task_info(start_info=self.start_info, end_info=end_info))
        else:
    ...
    def set_tasks_from_goal_info(
        ...
        self.goal = super().get_obs(modalities=self.goal_modalities)
        end_info = self.get_info()
        obs, info = super().reset(options=curr_state)
        self.start_info = self.get_info()
        self.selected_tasks = list(self.tasks.get_task_info(start_info=self.start_info, end_info=end_info))
        return obs, info
    ...
    def reset(self, seed=None, options=None):
        robot_obs = options.get("robot_obs", None) if options else None
        scene_obs = options.get("scene_obs", None) if options else None
        task_info = options.get("task_info", None) if options else None
        if robot_obs is not None or scene_obs is not None:
            self.selected_tasks = []
            # HACK: for observation_space compatibility
            self.goal = self.observation_space.spaces["goal"].sample()
            obs, info = super().reset(
                seed=seed,
                options={"robot_obs": robot_obs, "scene_obs": scene_obs},
            )
            self.start_info = self.get_info()
            return obs, info
    ...
    def set_camera_views_size(self):
        ...
        for camera in self.cameras:
            ...
            if depth_cam_name in self.modalities or depth_cam_name in self.goal_modalities:
                self.views_size[depth_cam_name] = (camera.height, camera.width)

    def get_observation_space(self):
        return spaces.Dict(
            {
                "observation": self.construct_observation_space_from_modalities(self.modalities),
                "goal": self.construct_observation_space_from_modalities(self.goal_modalities),
            }
        )

    def get_successful_tasks(self) -> List[str]:
        current_info = self.get_info()
        successful_tasks = self.tasks.get_task_info_for_set(self.start_info, current_info, self.selected_tasks)
