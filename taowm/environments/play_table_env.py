...
class PlayTableEnv(RLBaseEnv):
    def __init__(self, name: str = "play-table-v0", task: str = "open_drawer", **kwargs):
    ...
    def get_observation_space(self):
        ...
        if "scene_obs" in self.modalities:
            observation_space.spaces["scene_obs"] = spaces.Box(low=0, high=1, shape=(1,))
            self.scene_dim = 1
    ...
    def get_handle_position(self):
        ...
        for f_object in self.scene.fixed_objects:
            ...
            f_object_position = np.array(self.p.getLinkState(uid, link, physicsClientId=self.cid)[0])
    ...
    def get_target_joint(self):
        ...
        for door in self.scene.doors:
            if ("slide" in self.task and "slide" in door.name) or ("drawer" in self.task and "drawer" in door.name):
                joint_limits = self.p.getJointInfo(door.uid, door.joint_index, physicsClientId=self.cid)[8:10]
                target_state = self._normalize(door.get_state(), joint_limits[0], joint_limits[1])
                break
