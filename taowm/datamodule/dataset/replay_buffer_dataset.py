...
from taowm.utils.path_utils import get_file_list
...
class ReplayBufferDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.file_list = sorted(self.file_list, key=lambda file: int(file.stem.split("_")[-1]))
    ...
    def get_transf_obs(self, obs: dict, action: np.array = None):
        ...
        transf_obs = {modality: obs[modality] for modality in self.modalities if modality in obs}
        transf_obs = self.transform_manager(transf_obs, self.transf_type, device=self.device)
        return transf_obs
    ...
    def __getitem__(self, idx):
        ...
        if self.include_next_obs:
            item["obs"] = self.get_transf_obs(data["state"].item(), action=data["action"])
