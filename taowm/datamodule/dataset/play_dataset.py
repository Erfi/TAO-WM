...
from taowm.utils.episode_utils import get_state_info_dict
from taowm.utils.transforms import TransformManager
...
class PlayDataset(Dataset):
    ...
    def __init__(
        ...
        assert self.abs_datasets_dir.is_dir(), f"{str(self.abs_datasets_dir)} is not a dir"
        ...
        if include_goal:
            assert np.isclose(np.sum(list(goal_strategy_prob.values())), 1.0), "Goal strategy probability must sum to 1"
    ...
    def __getitem__(self, idx: Union[int, Tuple[int, int]]) -> Dict:
        if isinstance(idx, int):
            ...
            if self.min_window_size == self.max_window_size:
                ...
                logger.error(f"min_window_size {self.min_window_size} " f"> max_window_size {self.max_window_size}")
                raise ValueError
        else:
            idx, window_size = idx
        ...
        # Rearrange sequence
        states = {modality: sequence[modality] for modality in self.modalities if "action" not in modality}
        ...
        if self.include_goal:
            ...
            if goal_strategy == "geometric":
                ...
                seq["goal"] = self.get_similar_robot_obs_state(seq_start + window_size - 1)
                seq["disp"] = -1
        return seq
    ...
    def set_nn_steps_from_step(
        ...
        if data_type in nn_steps_from_step:
            self.nn_steps_from_step = {int(k): v for k, v in nn_steps_from_step[data_type].items()}  # maps str -> int
            return
    ...
    def get_file(self, file_idx, transform=True):
        ...
        data = load_npz(file_path)
        filtered_data = {modality: data[modality] for modality in self.modalities if "action" not in modality}
        if transform:
    ...
    def pad_sequence(
        ...
        # Update state info
        if not self.real_world:
            seq["state_info"] = {k: self.pad_with_repetition(v, pad_size) for k, v in seq["state_info"].items()}
    ...
    def zip_sequence(self, start_idx: int, end_idx: int, idx: int) -> Dict[str, np.ndarray]:
        ...
        """
        episodes = [load_npz(self.get_episode_name(file_idx)) for file_idx in range(start_idx, end_idx)]
    ...
    def set_ep_start_end_ids(self):
        ...
        if (self.abs_datasets_dir / "split.json").is_file():
            ...
            assert split_key in data_split, f"data split file doesn't contain {split_key} key"
            self.ep_start_end_ids = np.array(data_split[split_key])
            logger.info(f"Found 'split.json' with {len(self.ep_start_end_ids)} episodes.")
        else:
            self.ep_start_end_ids = np.load(self.abs_datasets_dir / "ep_start_end_ids.npy")
            logger.info(f"Found 'ep_start_end_ids.npy' with {len(self.ep_start_end_ids)} " "episodes.")
