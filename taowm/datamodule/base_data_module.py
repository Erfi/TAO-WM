...
from taowm.utils.episode_utils import load_dataset_statistics
...
class BaseDataModule(pl.LightningDataModule):
    def __init__(
        ...
        if (self.data_path / "training").is_dir():
            ...
            raise Exception(f"No training/validation partition found inside {str(self.data_path)}")

        self.transform_manager_cfg = OmegaConf.to_container(transform_manager, resolve=True)
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.train_dataset = hydra.utils.instantiate(train_dataset_cfg, data_dir=train_dir, train=True)
        ...
        if self.val_percentage > 0:
            ...
            self.val_dataset = hydra.utils.instantiate(val_dataset_cfg, data_dir=val_dir, train=False)
        ...
        # Vis dataset
        if self.create_vis_dataset:
            self.vis_dataset = hydra.utils.instantiate(vis_dataset_cfg, data_dir=val_dir, train=False)
...
@hydra.main(config_path="../../config", config_name="test/datamodule_test", version_base="1.3")
def main(cfg):
